...
var valueMaskName = Object.fromEntries(Object.entries(valueMask).map(function (_a) {
    var req = _a[0], masks = _a[1];
    return [req, Object.fromEntries(Object.entries(masks).map(function (_a) {
            var m = _a[0], _ = _a[1];
            return [_.mask, m];
        }))];
}));
function packValueMask(reqname, values) {
    ...
    /* numeric sort */
    masksList.sort(function (a, b) { return a - b; });
    var args = [];
...
exports.coreRequests = {
    ...
    GetWindowAttributes: [
        ...
        function (buf, backingStore) {
            ...
            var ret = { backingStore: backingStore };
    ...
    ResizeWindow: [
        // @ts-ignore
        function (win, width, height) { return exports.coreRequests.ConfigureWindow[0](win, {
            width: width,
            height: height
        }); }
    ],
    MoveWindow: [
        function (win, x, y) {
            // @ts-ignore
            return exports.coreRequests.ConfigureWindow[0](win, { x: x, y: y });
        }
    ],
    MoveResizeWindow: [
        function (win, x, y, width, height) {
            // @ts-ignore
            return exports.coreRequests.ConfigureWindow[0](win, { x: x, y: y, width: width, height: height });
        }
    ],
    RaiseWindow: [
        function (win) {
            // @ts-ignore
            return exports.coreRequests.ConfigureWindow[0](win, { stackMode: 0 });
        }
    ],
    LowerWindow: [
        function (win) {
            // @ts-ignore
            return exports.coreRequests.ConfigureWindow[0](win, { stackMode: 1 });
        }
    ],
    QueryTree: [
        ['CxSL', [15, 2]],
    ...
    ChangeProperty: [
        ...
        // format: 8/16/32
        function (mode, wid, atomName, atomType, units, data) {
    ...
    ListFonts: [
        function (pattern, max) {
            var reqLen = 2 + xutil_1.paddedLength(pattern.length) / 4;
    ...
    PutImage: [
        ...
        function (format, drawable, gc, width, height, dstX, dstY, leftPad, depth, data) {
            var padded = xutil_1.paddedLength(data.length);
        ...
        function (drawable, gc, x, y, items) {
            ...
            for (var i = 0; i < numItems; ++i) {
                ...
                if (typeof it_1 === 'string') {
                    if (it_1.length > 254) { // TODO: split string in set of items
                        throw new Error('not supported yet');
                    }
                    format += 'CCa';
                    args.push(it_1.length);
                    args.push(0); // delta???
                    args.push(it_1);
                    reqLen += 2 + it_1.length;
                }
                else {
                    throw new Error('not supported yet');
                }
            }
            var len4 = xutil_1.paddedLength(reqLen) / 4;
    ...
    QueryExtension: [
        function (name) {
            var padded = xutil_1.padded_string(name);
...
//# sourceMappingURL=corereqs.js.map